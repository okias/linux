				      ...
				      struct power_supply *psy)
{
	...
	if (psy->desc->type == POWER_SUPPLY_TYPE_USB) {
		if (!READ_ONCE(smb->usb_online))
			return POWER_SUPPLY_STATUS_DISCHARGING;
	} else {
		if (!READ_ONCE(smb->mains_online))
			return POWER_SUPPLY_STATUS_DISCHARGING;
	}
...
{
	/* Charing constraints */
	of_property_read_u32(np, "summit,max-chg-curr", &pdata->max_charge_current);
	of_property_read_u32(np, "summit,max-chg-volt", &pdata->max_charge_voltage);
	of_property_read_u32(np, "summit,pre-chg-curr", &pdata->pre_charge_current);
	of_property_read_u32(np, "summit,term-curr", &pdata->termination_current);
	...
	pdata->use_mains = of_property_read_bool(np, "summit,enable-mains-charging");
	pdata->use_usb = of_property_read_bool(np, "summit,enable-usb-charging");
	pdata->use_usb_otg = of_property_read_bool(np, "summit,enable-otg-charging");

	/* Enable charging method */
	of_property_read_u32(np, "summit,enable-chg-ctrl", &pdata->enable_control);
...
static struct of_device_id smb3xx_of_match[] = {
